...
public class bCoder {
   ...
   {
       String temp = new String(); int length=0;
       while (i<arr.length)
       {
           temp+=(char)arr[i];
           try {length=Integer.parseInt(temp);}
   ...
   {
       String temp = new String(); long val=0; boolean negative=false;
       boolean firstChar = true; boolean isZero=false;
       while (i<arr.length)
       {
           temp+=(char)arr[i];
           try{
               ...
               else if ((char)arr[i]=='-')
               {
                   ...
                   {negative=true; temp=new String();}
                   else
                       throw new InvalidBencodeException("Got Negative in incorrect place");
               }
               else
               {System.out.println(i);throw new InvalidBencodeException("Non Integer characters while reading int");}
           }
           i++;
       }
       return val;
   ...
   public static Parcel decode(byte arr[],ParcelType type) throws InvalidBencodeException
   {
       ...
       while (i<arr.length)
       {
           ...
           {
               case 'i':
                   if (kvmode!=KVMode.VALUE) throw new InvalidBencodeException("Expected String Got Integer for key");
                   i++;
                   stack.push('i');
               ...
               case 'd':
                   if (kvmode!=KVMode.VALUE) throw new InvalidBencodeException("Expected String Got KeyDictionary for key");
               ...
               case 'e':
                   ...
                   if (stack.peek()=='d' && kvmode == KVMode.VALUE) throw new InvalidBencodeException("KeyDictionary closed with a missing val");
                   ...
                   if (stack.peek()=='l')
                   {
                        ...
                        if (stack.peek()!='l')
                        {kvmode=KVMode.KEY; tempKey=KeyStack.pop(); tempKey="";}
                       ...
                       {kvmode=KVMode.KEY;tempKey=KeyStack.pop();}
                   }
                   else
                       stack.pop();
